...
export function stagesFromRecord(r) {
  ...
  const push = (stage, date, detail) => {
    if (!date) return;
    const event = { stage, category: categoryFromStage(stage), date, detail: detail || '', sourceUrl: r.DETAIL_LINK || '' };
...
export function toBill(record) {
  return {
    ...
    tags: tagBill({ committee: record.COMMITTEE || '', title: record.BILL_NAME }),
  };
}
