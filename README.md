...
3. **백필(`scripts/backfill-bills.mjs`, 2026-08-15 추가)** — 1·2번은 앞으로만 쌓여서
   과거 의안(파이프라인 시작 2026-08-13 이전 발의분)은 영영 안 잡힌다. 오너 지시
   "윤석열 정부 취임일(2022-05-10)부터 현재까지" 다 채운다 — AGE=21·22 목록을
   페이지네이션으로 훑어 `backfill-state.json`에 진행 상황을 남기고 회차마다
   이어간다. 규모가 커서(22대만 17,200건+) 완료까지 여러 날 걸린다.
